...
export default class Shapes {
        ...
        document.addEventListener("keydown", (e) => {
            if (e.key === "Delete") {
                ...
                lineMap.forEach((line, key) => {
                    if (line.isActive) {
                        if (line.startTo) {
                            const { rect, text, sphere } = this.getShape(
                                line.startTo
                            );
                            if (rect) {
    ...
    draw() {
        ...
        rectMap.forEach((rect) => {
            ...
            if (rect.isActive) {
                ...
                context.beginPath();
                context.moveTo(x + width * 0.5, y - this.tolerance);
                context.lineTo(x + width * 0.5, y - 20);
                context.stroke();
                context.closePath();
        ...
        lineMap.forEach((line) => {
            ...
            if (line.lineType === "straight") {
                ...
                if (line.curvePoints.length <= 2) {
                    ...
                    let lastPoint =
                        line.curvePoints[line.curvePoints.length - 1];
    ...
    getTransformedMouseCoords(event) {
        ...
        const mouseX =
            (event.clientX - rect.left + scrollBar.scrollPositionX) /
            Scale.scale;
        const mouseY =
            (event.clientY - rect.top + scrollBar.scrollPositionY) /
            Scale.scale;
    ...
    mouseDownDragAndResize(e) {
        ...
        rectMap.forEach((rect, key) => {
            ...
            const leftEdge =
                mouseX >= rect.x - this.tolerance && mouseX <= rect.x;
            const rightEdge =
        ...
        if (
            ...
            line &&
            (!smallestRect ||
                line.l.maxX - line.l.minX < smallestRect?.rect.width)
        ) {
        ...
        if (rectResize) {
            ...
            if (this.resizeElement.direction === null) {
                ...
                if (Math.abs(lineResize.curvePoints[0].y - mouseY) <= 10) {
                    lineResize.curvePoints[
                        lineResize.curvePoints.length - 1
                    ].y = lineResize.curvePoints[0].y;
                } else
                    lineResize.curvePoints[
                        lineResize.curvePoints.length - 1
                    ].y = mouseY;
            ...
            if (this.resizeElement.direction === "resizeStart") {
                if (line.startTo) {
                    ...
                    if (sphere && sphere.pointTo.length > 0) {
                        ...
                        if (
                            distance > sphere.xRadius ||
                            distance > sphere.yRadius
                        ) {
